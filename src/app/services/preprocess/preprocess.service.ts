...
const GradientQuestions : GradientMap = {
  "Q2": true,
  "Q3": false,
  "Q4": false,
  "Q5": false,
...
/**
 * Saves the data from the excel file excelPageDonnes
 */
  ...
  /**
 * Saves the questions form the excel file excelPageQuestions
 */
 ...
 */
  ...
  /**
 * Gets the most popular answer in data
 *
 * @param {QuestionData[]} questionData The data list
 * @returns {string} The most popular choice
 */
  ...
  /**
 * Gets all possible choices for a question
 *
 * @param {ExcelQuestions} question The question in question
 * @returns {string[]} All possible choices for that question
 */
 ...
 */
  ...
  /**
 * Checks if the question should be displayed as a gradient
 *
 * @param {string} symbol The symbol to analyze
 * @returns {boolean} True if it should one and false if it shouldn't
 */
  isGradientQuestion(symbol: string): boolean {
    let symbolStart = '';
    if(symbol.includes('n')){
      symbolStart = symbol.substring(0, symbol.indexOf('n'));
    } else if (symbol.includes('r')) {
      symbolStart = symbol.substring(0, symbol.indexOf('r'));
    } else {
      symbolStart = symbol;
    }
    return GradientQuestions[symbolStart];
  }
